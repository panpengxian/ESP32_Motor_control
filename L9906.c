...
void predrv_spi_Init(pmic_context_t** out_ctx)
{
    ...
    int re_code = spi_bus_initialize(SPI3_HOST, &spi_cfg, SPI_DMA_DISABLED);
    if(re_code == ESP_OK ){
...
void wr_predrv(pmic_context_t* ctx, int cmd, int data)
{
  char low_bit = data;
...
int rd_predrv(pmic_context_t* ctx, int cmd)
{
  int rxdata;
  spi_transaction_t spi_trans={
      .flags = SPI_TRANS_USE_RXDATA|SPI_TRANS_USE_TXDATA,
